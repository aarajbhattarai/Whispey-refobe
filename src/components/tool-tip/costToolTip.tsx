      ...
      <Tooltip.Root>
        <Tooltip.Trigger asChild>
          <span
            ...
            aria-label={`Total रु ${total.toFixed(2)}. Hover for cost breakdown.`}
          >
            रु {total.toFixed(2)}
          </span>
        </Tooltip.Trigger>
        <Tooltip.Portal>
          ...
          >
            {`LLM रु ${call.total_llm_cost?.toFixed(2)}
              TTS रु ${call.total_tts_cost?.toFixed(2)}
              STT रु ${call.total_stt_cost?.toFixed(2)}
              Plivo रु ${telecom.toFixed(2)} (${plivoRate.toFixed(2)}/min)`}
